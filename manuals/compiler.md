...
## GCP Distributed Docker Compiler



Instead of creating a docker swarm manually and host the autonomous systems one by one, we can use the GCP (Google Cloud Platform) Distributed Docker compiler to compile the emulation to a terraform plan; the plan will automatically create virtual machines for the swarm, configure the swarm, and deploy the autonomous systems on them.
...
Instead of generating the emulation files, this compiler gathers all graphable objects, creates the graphs, and generates the files that can be visualized using the `Graphviz` software. 
